...
public class UserGrpcImplementation extends sepServiceGrpc.sepServiceImplBase
{
    ...
    public void updatePassword(PasswordUpdateGrpcDTO request, StreamObserver<GenericMessage> responseObserver)
    {
        try
        {
            ...
            if (userRegistry.findById(request.getId()).isEmpty())throw new IllegalArgumentException("No user found in database!");
            var user=userRegistry.findById(request.getId()).get();
            if (request.getNewPassword().equals(user.getUserPass()))throw new IllegalArgumentException("New pass cant be same as old pass");
    ...
    public void updateUserInformation(UserUpdateGrpcDTO request, StreamObserver<GenericMessage> responseObserver)
    {
        try
        {
            if (userRegistry.findById(request.getId()).isEmpty())throw new IllegalArgumentException("User not found in the database!");
    ...
    @Override
    public void reportUser(GenericMessage request, StreamObserver<GenericMessage> responseObserver)
    {
        try
        {
            var newReport=new Report();
            newReport.setReportedUser(userRegistry.findById(Integer.parseInt(request.getMessage())).get());
